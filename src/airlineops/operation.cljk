(ns airlineops.operation
  "OperationActor -- one operations-coordination request = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (AirlineOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Aviation Safety Governor
  ...
  inner loop -- each request is auditable and checkpointed.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operations coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected). `:flag-flight-
  safety-concern` ALWAYS reaches this node when the governor is clean
  (never auto, at any phase) -- see `airlineops.phase`. `:schedule-
  flight-operation`/`:coordinate-maintenance` also always reach it when
  clean, at every phase this R0 enables."
...
(defn build
  ...
  `airlineops.store/Store`).
  opts:
    :advisor      -- an `airlineops.airlineopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (airlineopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AirlineOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (airlineopsllm/-advise advisor store request)]
            {:proposal p :audit [(airlineopsllm/trace request p)]})))

      ;; Aviation Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :flight-safety-flag
                                          :else :low-confidence))
                        :phase ph
